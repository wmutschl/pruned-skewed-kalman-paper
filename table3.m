% Replicates:
% - Table 3: "Time in seconds to compute 1000 evaluations of the log-likelihood function"
...
% Common options
SAMPLE_SIZES           = 50:50:250;          % loop over different sample lengths
OPT0.mc_replic         = 1000;               % number of Monte Carlo replications
...
if REDO == 1 || REDO == 3
    ...
    for sample_size = SAMPLE_SIZES
        ...
        tic
        [RES.Time,RES.SampleStats] = Computing_Time_Log_Likelihood(PARAMS,OPT); % simulates data and computes the log-likelihood function
        RES.time_Computing_Time = toc;
        OPT.filename = sprintf('results_computing_time_%s_T%d_R%d_%s',OPT.parameter_set,OPT.sample_size,OPT.mc_replic,OPT.computer_arch);
...
if REDO == 2 || REDO == 3
    ...
    for sample_size = SAMPLE_SIZES
        ...
        if sample_size > 100 && any(ismember(OPT.prune_tol,0))
            OPT.prune_tol(ismember(OPT.prune_tol,0)) = []; % remove the uncut version in multivariate case
            warning('In the multivariate case, we do not compute the unpruned version for sample sizes larger than 100. I remove the 0 in prune_tol.')
        end
        OPT.sample_size = sample_size;
        fprintf('%s with T=%d and R=%d\n',OPT.parameter_set,OPT.sample_size,OPT.mc_replic);
        tic        
...
DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP1_T50_R1000_'  OPT0.computer_arch],ONLY_LATEX);
DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP1_T100_R1000_' OPT0.computer_arch],ONLY_LATEX);
DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP1_T150_R1000_' OPT0.computer_arch],ONLY_LATEX);
DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP1_T200_R1000_' OPT0.computer_arch],ONLY_LATEX);
DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP1_T250_R1000_' OPT0.computer_arch],ONLY_LATEX);

DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP2_T50_R1000_'  OPT0.computer_arch],ONLY_LATEX);
DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP2_T100_R1000_' OPT0.computer_arch],ONLY_LATEX);
DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP2_T150_R1000_' OPT0.computer_arch],ONLY_LATEX);
DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP2_T200_R1000_' OPT0.computer_arch],ONLY_LATEX);
DisplayResults_MakeLatexTables_ComputingTime(['results_computing_time_DGP2_T250_R1000_' OPT0.computer_arch],ONLY_LATEX);
