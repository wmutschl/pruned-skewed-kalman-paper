...
OPT.optimizer.name = ["fminsearch"];
OPT.optimizer.optim_options = optimset('display','iter','MaxFunEvals',1000000,'MaxIter',6000,'TolFun',1e-8,'TolX',1e-6);
...
tic_id = tic;
[neg_log_likelihood,exit_flag] = negative_log_likelihood_dsge(xparam0,DATA.MAT,PARAM,MODEL,OPT);
    ...
    for jopt = 1:size(OPT.optimizer.name,2)
        ...
        switch OPT.optimizer.name(jopt)
            ...
            case 'cmaes' % does care about bounds
                ...
                [~, ~, ~, ~, ~, BESTEVER] = cmaes('negative_log_likelihood_dsge',xparam0,cmaesSIGMA,cmaesOptions,...
